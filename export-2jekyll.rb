...
WORDPRESS_XML_FILE_PATH = "plazamoyuacom.wordpress.2022-03-07.000.xml"
OUTPUT_PATH = "export2"
ORIGINAL_DOMAIN = "https://plazamoyua.com"

class Post
  attr_accessor :title, :post_date, :created_at, :slug, :url, :content, :textile_content, :author
  ...
  def initialize(item)
    ...
    @author = item.search("dc:creator").first.inner_text
  ...
  def save_comments(path)
    ...
    commies = comment_elements.collect { |el| Comment.new(self, el) }
    if commies.length > 0 then
      File.open("#{path}/#{created_at}-#{slug}.comments", "w") do |file|
        comment_elements.collect { |el| Comment.new(self, el) }.each { |comment| comment.write_to file }
      end
    end
    # File.open("#{path}/comments.json", "a") do |json_file|
    #   comment_elements.collect { |el| Comment.new(self, el) }.each { |comment| comment.write_to json_file }
    # end
  end
...
class Comment
  ...
  def initialize(post, element)
    ...
    _id = element.search("wp:comment_id").first.inner_text
    @id = Integer(_id)
    _parent_id = element.search("wp:comment_parent").first.inner_text
    @parent_id = Integer(_parent_id)
